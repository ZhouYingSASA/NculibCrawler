...
tree_url = "http://210.35.251.243/browse/cls_browsing_tree.php?s_doctype=all"


class NculibcrawlerSpider(scrapy.Spider):
    name = 'NcuLibCrawler'
    allowed_domains = ['210.35.251.243/browse/']
    start_urls = [list_url + "A1&page=27"]
    ...
    def parse(self, response):
        if response.url.startswith(list_url):  # 如果收到list url，爬取图书url，如果是最后一页，还yield下一个tree ch
            ...
            if len(title) > 3:
                ...
                if response.xpath('.//a[@class="blue"]/text()').extract()[-1] != "下一页":
                    yield self.make_requests_from_url(tree_url + "&cls=" + chr(ord(ch)+1) + "&lvl=1")
                else:
                    for url in urls:
            ...
            cls = response.xpath('//div[@class="stepright2"]/@name').extract()[0][4:]
            yield self.make_requests_from_url(list_url + cls)
